...
const useRiwayatStore = create((set) => ({
    ...
    fetchRiwayatSetor: async (uuid, token) => {
        set({ isLoading: true });
        try {
            const response = await fetch(`http://backend-laravel.mijelin.my.id/api/oil-transactions-byNIK?uuid=${uuid}`, {
    ...
    fetchRiwayatPenukaran: async (nik, token) => {
        set({ isLoading: true });
        try {
            const response = await fetch(`http://backend-laravel.mijelin.my.id/api/reward-redemptions/${nik}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            const data = await response.json();
